...
export default function PoLineItemsEditor({ lines, onChange }: PoLineItemsEditorProps) {
    ...
    const addLine = () => {
        ...
        const existingIndex = lines.findIndex((line) => line.productId === picker.productId);
        if (existingIndex >= 0) {
            const next = [...lines];
            next[existingIndex] = {
                ...next[existingIndex],
    ...
    return (
        <div className="space-y-3">
            <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Sản phẩm nhập</p>
                ...
                <div className="grid grid-cols-1 gap-2 sm:grid-cols-12 sm:items-end">
                    ...
                    <div className="sm:col-span-2">
                        ...
                        <input
                            ...
                            onChange={(e) => setPicker({ ...picker, quantity: Number(e.target.value) })}
                        />
                    </div>
            ...
            {lines.length > 0 ? (
                    ...
                    <table className="w-full text-sm">
                        ...
                        <tbody className="divide-y">
                            {lines.map((line, index) => {
                                ...
                                return (
                                    <tr key={`${line.productId}-${index}`} className="hover:bg-slate-50">
                ...
                <div className="rounded-lg border border-dashed px-4 py-6 text-center text-sm text-slate-400">
            ...
            {lines.length > 0 && (
                <p className="text-right text-xs text-slate-500">
                    {formatNumber(lines.length)} mặt hàng · {formatNumber(lines.reduce((s, l) => s + l.quantity, 0))} sản phẩm
                </p>
            )}
        </div>
    );
